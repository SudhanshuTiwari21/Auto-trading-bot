// Modified P2PB2B bot using fixed price adjustment approach
require('dotenv').config();
const { AutoTradingBot2 } = require('./bot2');
const P2PB2BExchange = require('./exchanges/p2pb2b');

class FixedAdjustmentP2PB2BBot extends AutoTradingBot2 {
    constructor() {
        super();
        console.log('Initializing Fixed Adjustment P2PB2B Trading Bot');
    }
    ...
    async executeTradeAction(action, symbol) {
        try {
            ...
            if (action === 'buy') {
                // For buy orders, adjust the ask price by a small fixed amount (0.001)
                const askPrice = parseFloat(marketPrice.ask);
                price = (askPrice - 0.001).toFixed(6);
                
                console.log(`Original ask price: ${askPrice}`);
                console.log(`Setting buy price to ${price} (0.001 below ask)`);
                
                // Calculate token amount based on USDT amount and price
                tokenAmount = (parseFloat(tradeAmount) / parseFloat(price)).toFixed(1);
            } else {
                // For sell orders, adjust the ask price by a small fixed amount (0.001)
                const askPrice = parseFloat(marketPrice.ask);
                price = (askPrice + 0.001).toFixed(6);
                
                console.log(`Original ask price: ${askPrice}`);
                console.log(`Setting sell price to ${price} (0.001 above ask)`);
...
async function runFixedAdjustmentBot() {
    try {
        console.log('=== Starting Fixed Adjustment P2PB2B Trading Bot ===');
        ...
        console.error('Error running Fixed Adjustment bot:', error.message);
    }
}

// Run the bot if this file is executed directly
if (require.main === module) {
    runFixedAdjustmentBot();
}

module.exports = { FixedAdjustmentP2PB2BBot };
