// Test script for P2PB2B market orders
...
// Test function to check market orders
async function testMarketOrders() {
    try {
        console.log('=== Testing P2PB2B Market Orders ===');
        ...
        // Create a matching sell market order
        console.log('\n4. Creating matching sell market order...');
        const sellOrder = await exchange.createMarketOrder('BRIL/USDT', 'sell', amount.toString());
        console.log('Sell market order created:', sellOrder);
        
        // Check status of buy order - should be filled by the market sell
        console.log('\n5. Checking buy order status after market sell...');
        ...
        console.error('Error in market order test:', error.message);
        if (error.response) {
