// Test script for P2PB2B bot with ask-based pricing approach
require('dotenv').config();
const { AskBasedP2PB2BBot } = require('./p2pb2b-ask-based');

async function testAskBasedBot() {
    try {
        console.log('=== Testing P2PB2B Bot with Ask-Based Pricing Approach ===');
        
        // Create bot instance
        const bot = new AskBasedP2PB2BBot();
        
        // Force a specific number of orders for testing
        bot.targetOrdersInCurrentCycle = 2; // Set to 2 orders for testing
        bot.ordersInCurrentCycle = 0;
        bot.currentCycle = 1;
        
        console.log('\nStarting test with 2 orders...');
        console.log('This will create orders with the ask-based pricing approach:');
        console.log('1. For BUY orders: Adjust ask price by reducing digits after 3rd decimal place by 1%');
        console.log('2. For SELL orders: Adjust ask price by increasing digits after 3rd decimal place by 1%');
        console.log('3. Immediately create matching order with same price to fill it');
        
        // Override the executeTrade method to stop after one cycle
        const originalExecuteTrade = bot.executeTrade;
        let cycleCompleted = false;
        
        bot.executeTrade = async function(exchange, symbol) {
            if (cycleCompleted) {
                console.log('\n=== Test completed successfully ===');
                console.log('Completed one full trading cycle with ask-based pricing approach');
                return;
            }
            
            // Call original method
            await originalExecuteTrade.call(this, exchange, symbol);
            
            // Check if cycle is completed
            if (this.ordersInCurrentCycle >= this.targetOrdersInCurrentCycle) {
                cycleCompleted = true;
                console.log('\n=== Full cycle test completed successfully ===');
                console.log(`Completed ${this.ordersInCurrentCycle} orders in cycle #${this.currentCycle}`);
                console.log('The bot is now using the ask-based pricing approach');
            }
        };
        
        // Start trading with P2PB2B exchange and BRIL_USDT symbol
        await bot.startTrading('P2PB2B', 'BRIL/USDT');
        
    } catch (error) {
        console.error('Error in ask-based bot test:', error.message);
    }
}

// Run the test
testAskBasedBot();
