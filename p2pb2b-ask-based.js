// Modified P2PB2B bot using ask-based pricing approach
require('dotenv').config();
const { AutoTradingBot2 } = require('./bot2');
const P2PB2BExchange = require('./exchanges/p2pb2b');

class AskBasedP2PB2BBot extends AutoTradingBot2 {
    constructor() {
        super();
        console.log('Initializing Ask-Based P2PB2B Trading Bot');
    }
    ...
    async executeTradeAction(action, symbol) {
        try {
            ...
            if (action === 'buy') {
                // For buy orders, adjust the ask price by 1% after the third digit
                const askPrice = parseFloat(marketPrice.ask);
                const integerPart = Math.floor(askPrice * 1000) / 1000; // Keep first 3 decimal places
                const fractionalPart = askPrice - integerPart;
                const adjustedFractionalPart = fractionalPart * 0.99; // Reduce by 1%
                price = (integerPart + adjustedFractionalPart).toFixed(6);
                
                console.log(`Original ask price: ${askPrice}`);
                console.log(`Setting buy price to ${price} (adjusted after 3rd decimal place)`);
                
                // Calculate token amount based on USDT amount and price
                tokenAmount = (parseFloat(tradeAmount) / parseFloat(price)).toFixed(1);
            } else {
                // For sell orders, adjust the ask price by 1% after the third digit
                const askPrice = parseFloat(marketPrice.ask);
                const integerPart = Math.floor(askPrice * 1000) / 1000; // Keep first 3 decimal places
                const fractionalPart = askPrice - integerPart;
                const adjustedFractionalPart = fractionalPart * 1.01; // Increase by 1%
                price = (integerPart + adjustedFractionalPart).toFixed(6);
                
                console.log(`Original ask price: ${askPrice}`);
                console.log(`Setting sell price to ${price} (adjusted after 3rd decimal place)`);
...
async function runAskBasedBot() {
    try {
        console.log('=== Starting Ask-Based P2PB2B Trading Bot ===');
        
        const bot = new AskBasedP2PB2BBot();
        ...
        console.error('Error running Ask-Based bot:', error.message);
    }
}
...
module.exports = { AskBasedP2PB2BBot };
